...
#[no_mangle]
pub unsafe extern "C" fn parse_and_write(notation: *const u8, notation_len: usize) {
    let conway_notation_token_tree = match ConwayPolyhedronNotationParser::parse(
        Rule::conway_notation_string,
        from_c_string(notation, notation_len),
    ) {
        Ok(c) => c,
        Err(e) => {
            println!("{}", e);
    ...
    token_tree.iter().for_each(|t| { match t.as_rule() {
        ...
        Rule::bevel => {
            ...
            let (regular, mut inner) = to_number(&mut inner);
            poly.bevel(
                ratio,
                height,
                Some(vertex_valence.unwrap().as_slice()),
                regular,
                true,
            );
        }
